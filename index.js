...
app.post("/expenses", (req, res) => {
  ...
  const expenseDateTime = DateTime
    ? new Date(DateTime).toISOString().slice(0, 19).replace("T", " ")
    : new Date().toISOString().slice(0, 19).replace("T", " ");
  const query =
    "INSERT INTO Expenses (ExpenseName, TotalAmount, DateTime) VALUES (?, ?, ?)";
  connection.query(
    query,
    [ExpenseName, TotalAmount, expenseDateTime],
    (err, results) => {
      if (err) throw err;
...
app.post("/api/inventory", (req, res) => {
  ...
  const sql =
    "INSERT INTO Inventory (ProductName, PricePerMeter, Quantity, TotalAmount, Remaining) VALUES (?, ?, ?, ?,?)";
  connection.query(
    sql,
    [productName, pricePerMeter, quantity, totalAmount, quantity],
    (err, result) => {
      if (err) {
        return res.status(500).json({ error: "Database query failed." });
      }
...
app.get("/orders", (req, res) => {
  const query =
    "SELECT c.CompanyOrderID AS CompanyID,c.CompanyName ,c.Address,c.PhoneNo FROM  CompanyOrders c ;";
  connection.query(query, (err, results) => {
    if (err) throw err;
    res.json(results);
  });
...
app.get("/orders/history/:companyID", (req, res) => {
  ...
  const getOrderIDsQuery =
    "SELECT OrderID FROM CompanyOrders WHERE CompanyOrderID = ?";
  ...
  connection.query(getOrderIDsQuery, [companyID], (err, results) => {
    ...
    const orderIDs =
      results[0]?.OrderID?.split(",").map((id) => parseInt(id, 10)) || [];
...
app.post("/placeorders", (req, res) => {
  ...
  if (!companyID || !productID || !quantity || !price) {
    return res
      ...
      .json({
        message:
          "All fields are required: companyID, productID, quantity, and price.",
      });
  }
  ...
  connection.query(
    query,
    [companyID, productID, quantity, price, totalPrice, orderDateTime],
    (err, result) => {
      if (err) {
...
app.post("/api/addClient", (req, res) => {
  ...
  const queryClient =
    "INSERT INTO CompanyOrders (CompanyName, PhoneNo, Address, PaymentID, TotalAmount, RemainingAmount,OrderID) VALUES (?, ?, ?, ?, ?, ?,?)";

  // Assuming you want to insert some default values for TotalAmount and RemainingAmount
  const totalAmount = 0.0; // default value for TotalAmount
  const remainingAmount = 0.0; // default value for RemainingAmount
    ...
    (err, result) => {
      if (err) {
        ...
        return res
          .status(500)
          .json({
            message: "Failed to add client to CompanyOrders table",
            error: err,
          });
      }
...
app.post("/api/placeOrder", (req, res) => {
  const { companyID, customerName, productID, quantity, price, paidAmount } =
    req.body;
  ...
  connection.beginTransaction((err) => {
    ...
    const getCompanyQuery =
      "SELECT companyName, address, phoneNo FROM CompanyOrders WHERE companyOrderID = ?";
    connection.query(getCompanyQuery, [companyID], (err, companyResult) => {
      ...
      const checkStockQuery =
        "SELECT Remaining,pricepermeter FROM Inventory WHERE ProductID = ?";
      connection.query(checkStockQuery, [productID], (err, stockResult) => {
        if (err) {
          return connection.rollback(() => {
            ...
            res
              .status(500)
              .json({ success: false, message: "Failed to check stock." });
          });
        }
        ...
        if (remainingStock < requestedQuantity) {
          return res
            ...
            .json({
              success: false,
              message: "Not enough stock available to place the order.",
            });
        }
        ...
        connection.query(
          ...
          (err, orderResult) => {
            ...
            const updateStockQuery =
              "UPDATE Inventory SET Remaining = Remaining - ? WHERE ProductID = ?";
          ...
          `;
                  connection.query(
                    updateCompanyOrdersQuery,
                    [totalBill - paidAmount, totalBill, companyName],
                    (updateCompanyOrdersErr, updateCompanyOrdersResult) => {
                      if (updateCompanyOrdersErr) {
                        console.error(
                          "Error updating company orders: ",
                          updateCompanyOrdersErr
                        );
                        return res
                          .status(500)
                          .json({
                            success: false,
                            message: "Failed to update company orders.",
                          });
                      }
                      return res
                        .status(201)
...
app.get("/api/getCompanyOrders", (req, res) => {
  const query =
    "SELECT CompanyName, Address, PhoneNo, RemainingAmount ,TotalAmount FROM CompanyOrders";
...
app.get("/api/getPaymentHistory/:CompanyName", (req, res) => {
  ...
  const query =
    "SELECT o.price,p.OrderID, p.AmountRemaining, p.DateTime, i.ProductName FROM  Payments p JOIN Orders o ON p.OrderID = o.OrderID JOIN Inventory i ON o.Product = i.ProductID WHERE  p.CompanyName = ?ORDER BY   p.DateTime DESC;";
...
app.post("/api/submitPayment", (req, res) => {
  ...
  const query =
    "SELECT TotalBill, PayedAmount,companyName FROM Orders WHERE OrderID = ?";
  connection.query(query, [orderID], (err, result) => {
    ...
    connection.query(
      ...
      (updateErr, updateResult) => {
        ...
        const companyQuery =
          "SELECT paid, AmountRemaining FROM Payments WHERE OrderID = ?";
        connection.query(
          companyQuery,
          [orderID],
          ...
          `;
            connection.query(
              updateCompanyQuery,
              [newRemainingAmount, newpaid, orderID],
              (updateCompanyErr, updateCompanyResult) => {
                if (updateCompanyErr) {
                  console.error(
                    "Error updating company order: ",
                    updateCompanyErr
                  );
                  return res
                    .status(500)
...
app.get("/api/getIncomeHistory/:CompanyName", (req, res) => {
  ...
  // Query to fetch the payment history for the given company ID
  const query =
    "SELECT o.OrderID, o.Product, o.PayedAmount, i.ProductName, o.DateTime, o.Income, ((o.PayedAmount /o.price )* (o.price - i.PricePerMeter)) AS ExtraPaid FROM Orders o JOIN  Inventory i ON o.Product = i.ProductID WHERE  o.CompanyName = ?ORDER BY  o.DateTime DESC;";
...
app.post("/api/returnPayment", (req, res) => {
  const { orderID, paymentAmount, qu } = req.body;
  ...
  // Query to fetch the TotalBill and PayedAmount for the order
  const query =
    "SELECT price,product,companyName,quantity FROM Orders WHERE OrderID = ?";
  connection.query(query, [orderID], (err, result) => {
    if (err) {
      console.error("Database error: ", err);
      return res
    ...
    const { price, product, companyName, quantity } = result[0];
    let quantity1 = paymentAmount / price;
    ...
    connection.query(
      ...
      (updateErr, updateResult) => {
        ...
        const priceQuery =
          "SELECT PricePerMeter from Inventory WHERE productID = ?";
        connection.query(priceQuery, [product], (companyErr, priceResult) => {
          if (companyErr) {
            console.error("Error fetching company data: ", companyErr);
            return res
              .status(500)
          ...
          const { PricePerMeter } = priceResult[0];
          const income = (price - PricePerMeter) * qu;
        ...
        `;
                  connection.query(
                    updateCompanyOrdersQuery,
                    [paymentAmount, paymentAmount, companyName],
                    (updateCompanyOrdersErr, updateCompanyOrdersResult) => {
                      if (updateCompanyOrdersErr) {
                        console.error(
                          "Error updating company orders: ",
        ...
        `;
                      connection.query(
                        updateOrdersquantityQuery,
                        [quantity1, paymentAmount, income, orderID],
                        (updateCompanyOrdersErr, updateCompanyOrdersResult) => {
                          if (updateCompanyOrdersErr) {
                            console.error(
                              "Error updating company orders: ",
